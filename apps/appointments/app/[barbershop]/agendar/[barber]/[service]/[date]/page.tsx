...
import { getConnection, sql } from "@barbearia.io/db";
...
async function getTimeSlots(options: { barberId: number; date: Date }) {
  ...
  const appointments = await getConnection()
    ...
    .innerJoin("EmployeeService", "EmployeeService.id", "Appointment.employeeServiceId")
  ...
  const availableTimeSlots = timeSlots.filter((timeSlot) => {
    const isAvailable = appointments.every((appointment) => {
      const diff = Math.abs(
        appointment.date.getTime() + timeToInteger(appointment.duration) - timeSlot.getTime(),
      );
      ...
      <ul>
        {timeSlots.map((timeSlot) => (
          <li key={timeSlot.getTime()}>
            <Link href={`/agendar/${barberId}/${params.service}/${params.date}/${format(timeSlot, "HH:mm")}`}>
